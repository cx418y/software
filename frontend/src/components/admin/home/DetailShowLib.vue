<template>
  <div class="article">
    ...
    <el-row class="indent">
      ...
      <div id="info" style="color: #f5f7fa">
      ...
      <div style="margin-left:150px;margin-top: 50px;max-height:60px;padding:15px;border-radius:10px;border: 1px solid #dedfe6">
        <label style="font-size: 15px;color: whitesmoke">评分：</label><br><br>
        <el-rate
          v-model="detail.rate"
          disabled
    ...
    <el-row class="intro">
      <h3 style="color: #f5f7fa">内容简介：</h3>
      {{detail.intro}}
    </el-row>
    <el-row class="comment">
      <h3 style="color: #f5f7fa">书评：</h3>
      ...
      <div class="comment-box" style="text-align: left" v-else>
        <div class="comment" v-for="item in commentList.slice((currentPage-1)*pagesize, currentPage*pagesize)" :key="item.commentId">
          ...
          <el-button @click="Delete(item.isbn,item.commentId)"  style="background-color: #f4f8f4;color: black;border-radius: 10px;font-family: 华光细圆_CNKI;width: 100px;float: right">删除评论</el-button>
